  ...
  return (
    <div className="divide-y divide-gray-200">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {userZones.find(uz => uz.isPrimary) && (
                <span className="text-sm text-gray-500">
                  {userZones.find(uz => uz.isPrimary)?.zone.name}
                </span>
              )}
            </div>
          </div>
            ...
            <tbody>
              {TIME_SLOTS.map(slot => (
                <tr key={slot.startTime}>
                  <td className="py-2 pr-4 text-gray-600 whitespace-nowrap text-sm">{slot.label}</td>
                  {DAYS.map((_, dayIndex) => {
      ...
      <div className="p-6">
        ...
        <p className="text-xs text-gray-500 mt-3">★ indicates your primary zone</p>
      </div>
